"""SMC / Strategy Setup Scanner Module

Encapsulates logic to scan a universe of symbols across multiple timeframes
and strategies to find active setups where the latest signal is not HOLD.

Backward compatibility:
  - ScannerSetup  is the canonical dataclass
  - SMCSetup      is an alias for ScannerSetup (legacy task files)
  - scan_smc_setups is an alias for scan_strategy_setups (legacy task files)

Import styles that all work:
    from delta_bt.scanner import scan_strategy_setups
    from delta_bt.scanner import scan_smc_setups
    from delta_bt.scanner.scanner import scan_strategy_setups
    from delta_bt.scanner.smc_scanner import scan_smc_setups   ← via smc_scanner.py shim below
"""
...
# Backward-compatible alias — legacy task files that reference SMCSetup still work
SMCSetup = ScannerSetup
...
def _check_symbol_strat(
    ...
    if trade_count == 0:
        logger.debug(
            f"Skipping {symbol} {strategy_name} {resolution}: no trades in 30d window"
        )
        return None
    ...
    try:
        # Prime strategy on all bars except the last
        ...
        logger.debug(
            f"7d signal evaluation failed for {symbol} {strategy_name}: {e}"
        )
        return None
...
def scan_strategy_setups(
    ...
    Returns:
        ...
        Each setup includes total_pnl, win_rate, trade_count from the 30d window.
    """
...
# Legacy task files that call scan_smc_setups() continue to work unchanged.
# New tasks should call scan_strategy_setups() directly.
# ---------------------------------------------------------------------------
def scan_smc_setups(
    client: DeltaClient,
    symbols: List[str],
    resolutions: List[str] = ["15m", "1h"],
    strategies: List[str] = ["smc_ob", "smc_ob_fvg", "smc_liquidity_sweep"],
    workers: int = 8,
    lookback_days: int = 7,
    profit_lookback_days: int = 30,
    min_win_rate: float = 0.50,
) -> List[ScannerSetup]:
    """Backward-compatible wrapper — delegates to scan_strategy_setups.

    Satisfies imports like:
        from delta_bt.scanner.smc_scanner import scan_smc_setups
        from delta_bt.scanner import scan_smc_setups
    """
    return scan_strategy_setups(
        client=client,
        symbols=symbols,
