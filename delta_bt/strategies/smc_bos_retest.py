...
Params:
  ...
  retest_window (int, 30) max bars to wait for a retest
  buffer_pct (float, 0.25) tolerance around the broken level
"""
...
@dataclass
class _Pending:
    direction: int   # +1 = bullish BOS, -1 = bearish BOS
...
class SmcBosRetest(Strategy):
    ...
    def on_bar(self, bar: Bar, ctx: StrategyContext) -> Signal:
        ...
        if self.last_hi is not None and bar.close > self.last_hi:
            self.pending.append(_Pending(+1, self.last_hi))
            self.last_hi = None
        if self.last_lo is not None and bar.close < self.last_lo:
            self.pending.append(_Pending(-1, self.last_lo))
            self.last_lo = None

        # Age and check retest — rebuild to avoid remove() hazard.
        tolerance = bar.close * (self.buf_pct / 100)
        signal = Signal.HOLD
        next_pending: list[_Pending] = []
        for p in self.pending:
            p.age += 1
            if p.age > self.window:
            ...
            fired = False
            if p.direction == +1:
                if bar.low <= p.level + tolerance and bar.close > p.level and self._state != 1:
                    self._state = 1
                    signal = Signal.BUY
                    fired = True
            else:
                if bar.high >= p.level - tolerance and bar.close < p.level and self._state != -1:
                    self._state = -1
                    signal = Signal.SELL
                    fired = True
            if not fired:
